...
func TestAwsMskIamMechanism(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.description, func(t *testing.T) {
			...
			mskMechanism := &aws_msk_iam.Mechanism{
			...
			expectedMap := map[string]string{
				...
				"host":    "localhost",
				//"user-agent":          aws_msk_iam.signUserAgent,
...
func TestDialKafka(t *testing.T) {
	// to produce messages
	topic := "my-topic"
	partition := 0
...
func TestDialKafka2(t *testing.T) {
	// to produce messages
